...
/* 	Function for WP All Import plugin
 *	Import all guest authors first then during post import
 * 	this will assign the guest authors to each post
 *  	Guest authors that do not exist will be created
 *	Called from wp-content/uploads/wpallimport/functions.php
 *	@R3N13R
 */
function set_post_coauthors( $post_id = 0, $author_str = '') {
	global $post, $post_ID, $coauthors_plus, $wpdb;

		$authors = array();
		$post_id = (int) $post_id;
		$failed = $created = '';
		$coauthors = explode(',', str_replace(' ', '', trim($author_str)));
		...
		foreach ( $coauthors as &$author_name ) {
			...
			if (empty($author->ID)){
				$name = str_replace ('cap-', '', $author_name);
				$name = ucwords(str_replace ('-', ' ', $name));

 				$requiredFields = $arrayName = array(
 					'display_name' => $name,
 					'user_login' => $name,
 					'description' => 'Guest Author');

				$id = $coauthors_plus->guest_authors->create($requiredFields);
		...
		wp_set_post_terms( $post_id, $authors, $coauthors_plus->coauthor_taxonomy, false );

		$failed = ($failed == '') ? '' : "Failed: " . rtrim($failed,',');
		$created = ($created == '') ? '' : "Created: " . rtrim($created,',');
		return  $failed . ' ' . $created;
}


/* 	Required for Newspaper V 9.5 theme
 *	Get guest authors posts called by td_ajax.php
 *	Used at Related posts - More From Author
 *	@R3N13R
 */
function get_coauthors_posts( $post_id = 0, $limit = 3, $page = 1) {
	...
	foreach ($authors as $key => $author) {
		...
		$postIdRes = $wpdb->get_results( "SELECT $wpdb->posts.ID FROM $wpdb->posts LEFT JOIN $wpdb->term_relationships AS tr1 ON ($wpdb->posts.ID = tr1.object_id) LEFT JOIN $wpdb->term_taxonomy ON ( tr1.term_taxonomy_id = $wpdb->term_taxonomy.term_taxonomy_id ) WHERE 1=1  AND (( ($wpdb->term_taxonomy.taxonomy = 'author' AND $wpdb->term_taxonomy.term_id = $term->term_id))) AND $wpdb->posts.post_type = 'post' AND ($wpdb->posts.post_status = 'publish' OR $wpdb->posts.post_status = 'private') GROUP BY $wpdb->posts.ID HAVING MAX( IF ( $wpdb->term_taxonomy.taxonomy = 'author', IF ( $wpdb->term_taxonomy.term_id = $term->term_id OR $wpdb->term_taxonomy.term_id = '',2,1 ),0 ) ) <> 1  ORDER BY $wpdb->posts.post_date DESC;");

		foreach ( $postIdRes as $postIdrow ) {
			$postIDary[] = $postIdrow->ID;
		}
	}

	// Order by post_date and get a mixed author slice of post Ids
	asort($postIDary);
	$found = sizeof($postIDary);
	$offset = ($page * $limit) - $limit;
	$output = array_slice( $postIDary , $offset, $limit);
	$IDs = implode(',', $output);

	// Now get the posts
	$postRes = $wpdb->get_results( "SELECT wp_posts.* FROM wp_posts WHERE ID IN ($IDs)");
	
	return array('posts' => $postRes, 'posts_found' => $found);
}


/* 	Modified function to allow specifying the post ID
 *	Required for Newspaper V 9.5 theme - called by wp-content/themes/Newspaper/includes/wp_booster/td_module.php
 *	@R3N13R
 *	
 ...
 */
function coauthors_posts_links_by_id( $id, $between = null, $betweenLast = null, $before = null, $after = null, $echo = true ) {
	...
	$coauthors_posts_links = coauthors__echo_by_id( $id, 'coauthors_posts_links_single', 'callback', array(
		'between'     => $between,
...
/* 	Modified function to allow specifying the post ID
 *	Required for Newspaper V 9.5 theme - called by wp-content/themes/Newspaper/includes/wp_booster/td_module.php
 *	Newspaper V 9.5 theme
 *	Helper function for the following new template tags
 *      @R3N13R 
 */
function coauthors__echo_by_id( $id, $tag, $type = 'tag', $separators = array(), $tag_args = null, $echo = true ) {
