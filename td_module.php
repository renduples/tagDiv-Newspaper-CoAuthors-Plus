    ...
    function __construct($post, $module_atts = array()) {
        ...
        if ( function_exists( 'get_coauthors_posts' ) ) {
            if (gettype($post) != 'object') {
                td_util::error(__FILE__, 'td_module: $post is not an object');
            }
        } else {
    ...
    function get_category() {
	    ...
	    if (array_key_exists($current_post_type, $builtin_post_types)) {
		    ...
		    if (!empty($td_post_theme_settings['td_primary_cat'])) {
                ...
                /*
                 * @MOD - 14 May 2019
                 * @R3N13R - START ignore old article category
			    if (is_category()) {
                ...
                foreach ($categories as $category) {
                    if ($category->term_id !== 2 AND $category->term_id !== 58546 AND $category->term_id !== 58554) {
                        $selected_category_obj = $category;
                        break;
                    }
                } // END ignore old article category
